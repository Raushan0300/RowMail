    ...
    <div className={`flex flex-col justify-between w-full sm:w-[75%] bg-white h-screen py-2.5`}>
      {loading?<div className="flex justify-center items-center h-full"><CircularProgress /></div>:<div className="flex flex-col h-full">
        ...
        <div className="flex gap-5 items-center py-2.5 px-5 border-b pb-2">
          <ArrowBackIcon className="cursor-pointer text-[#8A95AD] text-[18px]" onClick={()=>{setMessageId('')}}/>
        ...
        <div className="mt-2 px-5 border-b-2">
          ...
          <Image src="/vercel.svg" alt="" width={40} height={40} />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <h3 className="text-[10px] sm:text-[12px] text-[#8A95AD]">{messageData?.from?.match(/^(.*)\s<(.+)>$/)?.[2]}</h3>
            </div>
            <h3 className="text-[10px] sm:text-[12px] text-[#8A95AD]">To: <span className="text-[#0E0E23]">{messageData.to}</span></h3>
            </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-5 items-center">
              <span className="text-[12px] text-[#8A95AD]">{formattedDate}</span>
              <div className="flex gap-3 items-center">
              <Image src="/reply.svg" alt="" width={20} height={20} className="cursor-pointer" />
              <Image src="/forward.svg" alt="" width={20} height={20} className="scale-x-[-1] cursor-pointer" />
              <Image src="/star.svg" alt="" width={20} height={20} className="cursor-pointer" />
              </div>
            </div>
          </div>
        </div>
